...
-- [임시 주소] 본인의 깃허브 Raw 주소로 변경해야 합니다. (끝에 / 포함)
local GITHUB_RAW_URL = "https://raw.githubusercontent.com/YOUR_GITHUB_NAME/YOUR_REPO_NAME/main/"

-- ===========================================
-- 🔄 자동 업데이트 시스템
-- ===========================================
function CheckUpdate()
    if GITHUB_RAW_URL == "https://raw.githubusercontent.com/YOUR_GITHUB_NAME/YOUR_REPO_NAME/main/" then return end
    
    local res = gg.makeRequest(GITHUB_RAW_URL .. "version.txt")
    if res and res.content then
        local remoteVer = res.content:match("^%s*(.-)%s*$") -- 공백 제거
        if remoteVer and remoteVer ~= SCRIPT_VERSION then
            local choice = gg.alert("새로운 버전(v" .. remoteVer .. ")이 출시되었습니다!\n업데이트 하시겠습니까?", "예", "아니요")
            if choice == 1 then
                gg.toast("다운로드 중...")
                local newScript = gg.makeRequest(GITHUB_RAW_URL .. "bc.lua")
                if newScript and newScript.content then
                    local path = gg.getFile()
                    local file = io.open(path, "w")
                    if file then
                        file:write(newScript.content)
                        file:close()
                        gg.alert("✅ 업데이트 완료!\n스크립트를 다시 실행해주세요.")
                        os.exit()
                    else
                        gg.alert("❌ 파일 쓰기 권한이 없습니다.")
                    end
                else
                    gg.alert("❌ 다운로드 실패!")
                end
...
-- ===========================================
function KeyLogin()
    if GITHUB_RAW_URL == "https://raw.githubusercontent.com/YOUR_GITHUB_NAME/YOUR_REPO_NAME/main/" then
        gg.toast("⚠️ 임시 주소 상태입니다. 키 로그인을 건너뜁니다.")
        return true
    end
    ...
    if file then
        ...
        local keysRes = gg.makeRequest(GITHUB_RAW_URL .. "keys.txt")
        if keysRes and keysRes.content and string.find(keysRes.content, savedKey) then
    ...
    local inputKey = prompt[1]
    
    -- 3. 깃허브 서버에서 키 검증
    gg.toast("키 검증 중...")
    local keysRes = gg.makeRequest(GITHUB_RAW_URL .. "keys.txt")
    
    if keysRes and keysRes.content and string.find(keysRes.content, inputKey) then
