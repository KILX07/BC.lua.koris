...
function CheckUpdate()
    ...
    local res = gg.makeRequest(GITHUB_RAW_URL .. "version.txt" .. cacheBuster)
    if res and res.content then
        local remoteVer = res.content:match("^%s*(.-)%s*$") -- 공백 제거
        if remoteVer and remoteVer ~= SCRIPT_VERSION then
            -- 사용자에게 묻지 않고 몰래 다운로드 진행
            gg.toast("🔄 최신 버전 확인 중...")
            local newScript = gg.makeRequest(GITHUB_RAW_URL .. "bc.lua" .. cacheBuster)
            
            if newScript and newScript.content then
                -- [핵심] 다운로드한 깃허브 bc.lua 파일 안의 버전이 version.txt와 똑같은지 검사 (무한루프 방지)
                local downloadedVer = newScript.content:match('local SCRIPT_VERSION%s*=%s*"([^"]+)"')
                
                if downloadedVer == remoteVer then
                    local path = gg.getFile()
                    local file = io.open(path, "w")
                    if file then
                        file:write(newScript.content)
                        file:close()
                        gg.alert("✅ 최신 버전(v" .. remoteVer .. ")으로 자동 업데이트 되었습니다!\n스크립트를 다시 실행해주세요.")
                        os.exit()
                    end
                else
                    -- 깃허브의 bc.lua 파일 안의 버전이 아직 안 바뀌었을 경우 덮어쓰지 않고 무시
                    gg.toast("⚠️ 서버 동기화 대기 중... (업데이트 보류)")
                end
            end
        end
    end
...
function KeyLogin()
        ...
        if savedKey then
            ...
            if savedKey ~= "" then
                ...
                if keysRes and keysRes.content then
                    if string.find(keysRes.content, savedKey, 1, true) then
                        gg.toast("✅ 자동 로그인 성공! (IP: " .. myIP .. ")")
                        return true
                    else
                        gg.alert("⚠️ 인증이 만료되었거나 키가 변경되었습니다.\n다시 로그인해주세요.")
    ...
    if keysRes and keysRes.content and string.find(keysRes.content, inputKey, 1, true) then
